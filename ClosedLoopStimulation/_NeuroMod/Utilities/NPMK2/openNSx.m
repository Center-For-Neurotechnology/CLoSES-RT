...
NSx.MetaTags.openNSxver = '5.1.11.0';
...
% invalid packet
% EDIT: added length check on NSx.MetaTags.Timestamp to the below conditional
if length(NSx.MetaTags.DataPoints) > 1 && length(NSx.MetaTags.Timestamp) > 1 && all(NSx.MetaTags.Timestamp(1:2) == [0,0])
...
% EDIT: added the "intersect" call inside the index to stay within the bounds of the vector
if any(NSx.MetaTags.DataPoints == 0) && strcmp(ReadData, 'read')
    ...
    NSx.MetaTags.Timestamp(intersect(segmentsThatAreZero,1:length(NSx.MetaTags.Timestamp))) = [];
    NSx.Data(intersect(segmentsThatAreZero,1:length(NSx.Data))) = [];
end
