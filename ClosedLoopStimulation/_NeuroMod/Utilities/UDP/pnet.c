...
BRAINGATE NOTES:
on Windows Matlab, compile using:
mex -O pnet.c ws2_32.lib winmm.lib -DWIN32 -largeArrayDims

on Ubuntu Matlab:
max -g -v -largeArrayDims pnet.c

added IP arg to UDPSOCKET command to force
reads to bind to a specific address.
- N.Schmansky, Nov 2012

added setsockopt to tcp_udp_sock to increase both read
and write buffer sizes
- N.Schmansky, Mar 2013

added SOCKET_ERROR define for linux compile
- N.Schmansky, Dec 2013
...
void Print_Start_Message()
{
  ...
  mexPrintf("\n===============================================================================\n"
            ...
            "GNU General Public License, see license.txt for full license notes.\n"
...
void byteswapcopy(char *dest,char *src,const int elements,const int elementsize,int mode)
{
  /* MODE=0 Do nothing,
     MODE=1 Swap,
     MODE=2 network byte order,
     MODE=2 Intel byte order.
     A little smart ckeck of byte the machine byte order. */
  const int ordertest=1;
  const char *is_intel_order=(const char *)(&ordertest);
  if(is_intel_order[0]==1 && mode==2)
...
*/
int tcp_udp_socket(int port,int dgram_f, const char* inet_addr_s, int rbuf_size_mb)
{
  int sockfd;
...
#define IPTOS_LOWDELAY          0x10
  ...
  if(sockfd==-1)
  {
    mexPrintf("pnet: tcp_udp_socket: socket fail!\n");
    return -1;
  }
  ...
  if (inet_addr_s)
  {
    my_addr.sin_addr.s_addr = inet_addr(inet_addr_s);
    if (my_addr.sin_addr.s_addr==INADDR_NONE)
    {
      mexPrintf("pnet: Can't resolve host string %s as dot notation IP number\n",inet_addr_s);
      close(sockfd);
      return -1;
    }
    ...
    {
      mexPrintf("pnet: Couldnt set sockopt for broadcast\n");
      close(sockfd);
      return -1;
    }
  }
...
#if 1 /* added by N.Schmansky to fix dans UDP2Struct buffer overflow problem
         Set the data stream input buffer size to user specified,
         typically something very large (64MB), but default is 1MB */
{
    int opt_len = sizeof(int);
    int data_buff_size = rbuf_size_mb*1024*1024;
    if (setsockopt(sockfd, SOL_SOCKET, SO_RCVBUF,
                 (char*)&data_buff_size, opt_len) == SOCKET_ERROR)
    {
    mexPrintf("pnet: tcp_udp_socket: setsockopt set input buffer size fail!\n");
    close(sockfd);
    return -1;
    }

    /* Set the data stream output buffer size to 128KB */
    opt_len = sizeof(int);
    data_buff_size = 128*1024;
    if (setsockopt(sockfd, SOL_SOCKET, SO_SNDBUF,
                 (char*)&data_buff_size, opt_len) == SOCKET_ERROR)
    {
    mexPrintf("pnet: tcp_udp_socket: setsockopt set output buffer size fail!\n");
    close(sockfd);
    return -1;
    }
}
  ...
  {
    mexPrintf("pnet: tcp_udp_socket: bind fail!\n");
    close(sockfd);
    return -1;
  }
...
)
{
  ...
  if(mxIsChar(my_mexInputArg(0)))
  {
    ...
    {
      const int fd=
        tcp_udp_socket((int)my_mexInputScalar(1), 0, NULL, 1);
      if(fd>=0)
      {
        move_con(STATUS_FREE);
    ...
    if((myoptstrcmp(fun,"UDPSOCKET")==0) ||
       (myoptstrcmp(fun,"UDPLARGESOCKET")==0))
    {
      int fd = -1;
      int rbuf_size_mb = 1; /* default 1MB receive buffer */
      if (myoptstrcmp(fun,"UDPLARGESOCKET")==0)
      {
	/* large receive buffer, like 64MB */
          rbuf_size_mb = (int)my_mexInputScalar(3);
      }
      fd = tcp_udp_socket((int)my_mexInputScalar(1), 1,
                           my_mexInputOptionString(2), rbuf_size_mb);
      if(fd>=0)
      {
        move_con(STATUS_FREE);
