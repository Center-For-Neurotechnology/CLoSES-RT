function [sumCoherencePerChannel, avCoherence, coherenceValue, pairChannels] = runCoherence_EML(filteredData, lowFreq, highFreq, Fs, detectChannelInds)
%#codegen
...
freqBins = linspace(lowFreq, highFreq, nFreqs);
...
options.Fs = Fs;
filteredDataSlow = downsample(filteredData,25); %round(Fs/(highFreq*10))); % keep 10 times highest frequency (could be less)
