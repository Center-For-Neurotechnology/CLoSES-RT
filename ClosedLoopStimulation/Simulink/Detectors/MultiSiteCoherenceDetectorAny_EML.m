function [eventDetected, eventPropagated, stimChannelNumber] = MultiSiteCoherenceDetectorAny_EML(activeBool, signalIn, indDetPairCh, thresholdsUpperLim, thresholdsLowerLim, nDetectionsRequired, possibleStimChannels, probabilityOfStim)
...
if activeBool && any(thInDetChUpperLim > eps)
    ...
    if any(sigInDetCh > thInDetChUpperLim)  %Larger than threhold and sign is positive
 %       eventDetected = true;
        nEventDetectedPrevUpper = nEventDetectedPrevUpper + 1;
    elseif any(sigInDetCh < thInDetChLowerLim) %Smaller than threhold and sign is negative
 %       eventDetected = true;
