function [eventDetected, eventPropagated] = TxThresholdCrossingDetectorAll_EML(activeBool, signalIn, detectableChannels, channelThreshold, nDetectionsRequired, txSign, probabilityOfStim)

%#codegen
%Detects if ALL signalIn are larger than their corresponding channelThreshold (or smaller depending on txSign) 
...
%
% Almost identical to TxThresholdCrossingDetector_EML.m but with ALL instead of ANY
% Which one runs is selected in TxThresholdCrossingDetector.m
...
if activeBool && all(channelThreshold(detectableChannels) > eps)
    if (txSign > 0) && all(signalIn(:,detectableChannels) > channelThreshold(detectableChannels))  %Larger than threhold and sign is positive
      %  eventDetected = true;
        nEventDetectedPrev = nEventDetectedPrev + 1;
    elseif (txSign < 0) && all(signalIn(:,detectableChannels) < channelThreshold(detectableChannels)) %Smaller than threhold and sign is negative
    %    eventDetected = true;
        nEventDetectedPrev = nEventDetectedPrev + 1;
    else
