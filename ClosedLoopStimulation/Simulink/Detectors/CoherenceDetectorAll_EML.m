function [eventDetected, eventPropagated] = CoherenceDetectorAll_EML(activeBool, signalIn, indDetPairCh, thresholds, nDetectionsRequired, txSign, probabilityOfStim)
...
if activeBool && any(thInDetCh > eps)
    ...
    if (txSign > 0) && all(sigInDetCh > thInDetCh)  %Larger than threhold and sign is positive
    %    eventDetected = true;
        nEventDetectedPrev = nEventDetectedPrev + 1;
    elseif (txSign < 0) && all(sigInDetCh < thInDetCh) %Smaller than threhold and sign is negative
    %    eventDetected = true;
        nEventDetectedPrev = nEventDetectedPrev + 1;
    else
        nEventDetectedPrev = 0;
