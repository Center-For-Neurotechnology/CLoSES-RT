function [sCoreParams, variantConfig] = ConfigurationFile_FromPreviousSession_Continuous_IIDs_P12(sCoreParams, variantConfig)
% This file contains the specific configuration for an experiment during SLEEP
% The idea is to detect sleep spindles and send stimulation trigger after 200samples
%
% In this case we specify the fileName of the  original experiment and take
% the configuration from there.
%
% It is useful to replay with the EXACT same configuration of an experiment
% and to later modify one parameter at a time
%
% This configuration is to trigger stimulation when power in "Spindle" band (12-16Hz) *increases*
...
%
% Variant Selection:
% - freqBandName = 'SPINDLES';          % Options: THETA (4-8) / ALPHA (8-15) / BETA (15-30) / LOWGAMMA (30-55) / HIGHGAMMA (65-110) / HIGHGAMMARIPPLE (65-200) / RIPPLE (140-200) / GAMMA (30-110) /SPINDLES (12-16) / NOFILTER
...
% @Rina Zelmann 2016-2019

%%%%%%%%%%
%% Load File from original experiment
% This MUST be written here for each patient
dirGral = 'E:\DATA\ClosedLoopPhysiology\Patients'; 
pName='P12';
dirGralData = [dirGral, filesep, pName, filesep, 'ClosedLoopPhysiology_IIDs_STIM'];
fileNameOriginalExperiment = [dirGralData,filesep, 'P12_Decider_Data',filesep, 'P12_IIDs_3detRPH1_stimRPH23_sleep',filesep,'DeciderData_CONTINUOUS_P12_IIDs_RHP1_sleep_ds1_2308','.mat'];

load(fileNameOriginalExperiment, 'SessionDataConfig');
sCoreParams = SessionDataConfig.sCoreParams;
...
freqBandName = 'NOFILTER';         % Options: THETA (4-8) / ALPHA (8-15) / BETA (15-30) / LOWGAMMA (30-55) / HIGHGAMMA (65-110) / HIGHGAMMARIPPLE (65-200) / RIPPLE (140-200) / GAMMA (30-110) /SPINDLES (12-16) / NOFILTER
...
%sCoreParams.Features.Baseline.initialThresholdValue = 10;  % Threshold is really this value x sCoreParams.decoders.txDetector.txRMS - Could be used to specify a fix or initial value of threshold (e.g.: to have same value during the whole experiment specify value here and set sCoreParams.Features.Baseline.weightPreviousThreshold=1)
...
% sCoreParams.decoders.txDetector.nDetectionsRequestedmSec = 250;   % duration in ms of consecutive detections required to produce a stiulation (idea of only detecting if feature is large for a certain duration)
...
% sCoreParams.decoders.txDetector.ProbabilityOfStim = .99;        % Given a detected event, what is the proba of actually sending a stimulation pulse

%%%%%%%%%

%% Stimulation Parameters
stimulationType = 'REALTIME';                   % Options: REALTIME / NEXTTRIAL
% sCoreParams.stimulator.startupTimeSec = 5;                 % Wait in seconds before allowing stimulation
...
%sCoreParams.stimulator.amplitude_mA = 4;        % Stimulation amplitude (1mA) - not used but useful to have in saved Data

%%%%%%%%%%

%% Visualization
sCoreParams.viz.channelInds = 1:sCoreParams.decoders.txDetector.nChannels;  %First channels - can be changed afterwards keeping the same number
